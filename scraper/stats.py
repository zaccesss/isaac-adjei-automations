...
def record_stat(ctx, source: str, rows: int, note: str = "") -> None:
    # append to the run context rather than returning so every scraper function
...
def write_summary(ctx) -> None:
    # write a Markdown summary table to source-stats.md so the workflow can cat it
    ...
    for stat in ctx.source_stats:
        # coerce None note to empty string for cleaner table output.
