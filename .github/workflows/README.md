...
Every scheduled job has two trigger paths. The primary is a **dispatch from the cron-ops worker**, which
lands at the job's exact UK wall-clock time. Behind it sits a thin **GitHub Actions cron backup** in the
workflow file, kept because a worker outage should never silence a job. GitHub Actions is UTC only, does
not observe British Summer Time and delays or drops scheduled runs (worst at the top of the hour), so the
cron is the safety net rather than the primary. Whichever path fires first gates itself to the intended
UK hour and claims the day, so the job acts exactly once. See
[UK time and the windowed schedule](#uk-time-and-the-windowed-schedule).

The table lists both times: the worker dispatch (UK) and the GitHub Actions backup cron (UTC, since that
is what the workflow file holds).
...
| [daily-alerts](daily-alerts.yml) | 07:00-10:00 | 06:17 + 06:47 + 07:17 + 07:47 | [`daily-alerts.mjs`](../../scripts/daily-alerts.mjs) | Posts a daily Bible verse, university deadlines closing in and library books due back, each to its own channel |
| [daily-analytics](daily-analytics.yml) | 01:00 | 02:23 + 02:53 | [`daily-analytics.mjs`](../../scripts/daily-analytics.mjs) | Posts a per-page analytics summary (Applications, Posts, Fitness, Music) to each dashboard analytics channel |
| [recategorise](recategorise.yml) | 05:00 | 06:11 + 06:41 | [`recategorise.py`](../../scripts/recategorise.py) | Re-categorises "Software Engineering" catch-all applications with the AI (idempotent mop-up) |
...
hour. So instead of one fragile slot, each daily job keeps **a backup pair of cron tries** (two runs
...
The all-day jobs (medication, reminders, Spotify) do not gate to an hour: they run through the day and
