...
Each job runs from an exact-time dispatch with a thin cron backup in the workflow files; the windowed
jobs additionally gate on the Europe/London hour so they land at the right local time year round. The
...
Geocode locations deliberately has no Healthchecks check of its own: it is low-stakes (a missed run just leaves some map pins stale a bit longer),
