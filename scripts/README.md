...
| --- | --- | --- | --- |
| [`wakatime-sync.py`](wakatime-sync.py) | Python | wakatime-sync, daily-coding-summary | Fetches WakaTime daily totals, per-hour durations, categories, machines, dependencies and extra coding metrics and upserts them into `wakatime_daily` (`on_conflict=date`, so re-runs are safe) |
